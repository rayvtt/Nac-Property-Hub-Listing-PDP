...
- **Injects a managed SEO/GEO `<head>` block** on every sync (`buildManagedSeoHead` / `stripManagedSeoHead`). It strips any prior managed tags (scoped to `<head>` only — body JSON-LD like the case-study Article `ItemList` is preserved), then re-injects a fresh, country-specific, listing-aware set: `canonical` + `og:url` (from `🔗 Country URL`, else built from `wpSlug`), `og:locale` vi_VN + en_US alternate, `og:site_name`, full Twitter card, `hreflang` vi/en/x-default, `robots`, and a JSON-LD `@graph` of **CollectionPage + BreadcrumbList + ItemList**. The ItemList renders every listing as a `Product` (name, image, url, brand, category, `Offer` with price + currency) — this is the GEO/LLM citability layer so AI overviews can extract the actual properties + prices. Idempotent: strip-then-inject means re-sync never duplicates, and all countries (current + future scaffolds) converge to identical structure. The in-place `<title>` / `meta description` / `og:title` / `og:description` / `og:image` updates remain as before.
