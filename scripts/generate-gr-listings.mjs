...
function buildProps(p) {
  ...
  return {
    ...
    '📝 Desc EN': txt(`${p.name} is a EUR-priced ${D.city === 'Galaxidi' ? 'Corinthian-Gulf' : 'Athens'} ${typeEn} eligible for Greece’s Golden Visa residence-by-investment programme. ${D.en} Units start at ${eur(entry)} (${entryBandEn}), delivered furnished with an optional developer prepaid-rent programme. The Golden Visa grants the whole family a 5-year renewable EU residence permit with Schengen free movement and no minimum-stay requirement. ${p.handoverEn}.`),
